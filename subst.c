...
static WORD_LIST *call_expand_word_internal __P((WORD_DESC * ,
                                                        int, int, int *, int *));
...
static wchar_t *remove_wpattern __P((wchar_t * , size_t, wchar_t *,
                                            int));
...
static int match_wpattern __P((wchar_t * ,
                                      char **, size_t, wchar_t *, int, char **, char **));
...
static char *list_remove_pattern __P((WORD_LIST * ,
                                             char *, int, int, int));
...
static char *array_remove_pattern __P((SHELL_VAR * ,
                                              char *, int, char *, int));
...
static int verify_substring_values __P((SHELL_VAR * ,
...
static WORD_LIST *expand_word_internal __P((WORD_DESC * ,
                                                   int, int, int *, int *));
...
static WORD_LIST *glob_expand_word_list __P((WORD_LIST * ,
                                                    int));

#ifdef BRACE_EXPANSION

static WORD_LIST *brace_expand_word_list __P((WORD_LIST * ,
                                                     int));
...
static WORD_LIST *shell_expand_word_list __P((WORD_LIST * ,
                                                     int));

static WORD_LIST *expand_word_list_internal __P((WORD_LIST * ,
                                                        int));
        ...
        char **sp, **ep;
{
    ...
    if (extended_glob)
        simple &= (wpat[1] != L'(' ||
                   (wpat[0] != L'*' && wpat[0] != L'?' && wpat[0] != L'+' && wpat[0] != L'!' && wpat[0] != L'@')); /*)*/
#endif
...
#  endif /* CASEMOD_CAPCASE */
    ...
    if (check_nullness)
        var_is_null |=
                var_is_set && var_is_special && (quoted & (Q_HERE_DOCUMENT | Q_DOUBLE_QUOTES)) && QUOTED_NULL (temp);
...
//    fprintf(stderr, "\x1B[33m" " >>> EXPANDING WORD: [%s]\x1B[0m\n", string);
    ...
    for (sindex = 0; ;) {
        ...
        switch (c) {
	       ...
	       and consume input. */
                ...
                fprintf(stderr, "\x1B[33m" " >>> PROCESS SUBSTITUTION: [ %c(%s) ] -> [ %s ]\x1B[0m\n", c, temp1, temp);
	    ...
	    word->flags |= W_ITILDE;
	     ...
	     assignment statement, we don't do tilde expansion. */
                ...
                if (temp && *temp && t_index > 0) {
                    ...
                    free(temp);
                    fprintf(stderr, "\x1B[33m" " >>> TILDE EXPANSION: [ %s ] -> [ %s ]\x1B[0m\n", temp, temp1);
                    temp = temp1;
                    sindex += t_index;
            ...
            case '$':
                ...
                fprintf(stderr, "\x1B[33m" " >>> PARAMETER EXPANSION: [ %.*s ] -> [ %s ]\x1B[0m\n", sindex - prev_sindex, string + prev_sindex, temp);
	     ...
	     expand_word_internal if nothing else in the string */]
	       ...
	       ` to pass through, for backwards compatibility. */
                ...
                if (word->flags & W_NOCOMSUB)
                    ...
                    fprintf(stderr, "\x1B[33m" " >>> COMMAND SUBSTITUTION: [ `%s` ] -> [ %s ]\x1B[0m\n", temp, temp1);
	     ...
	     whole word was quoted. */
		     ...
		     for us because of the way it prints error messages. */
                    ...
                    if(tword && tword->word && list && list->word && list->word->word) {
                        fprintf(stderr, "\x1B[33m" " >>> QUOTED: [ %s ] -> [ %s ]\x1B[0m\n", tword->word, list->word->word);
                    }
